...
export default function Withdraw() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setResult(res.data)
      setForm(f => ({ ...f, amount: '' }))
      setSelectedAccount(prev => ({ ...prev, balance: res.data.balanceAfter }))
    } catch (err) {
        ...
        <div className="card p-6">
          ...
          {result && (
            <div className="mb-5 p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="font-semibold text-green-700 text-sm mb-1">Withdrawal Successful!</p>
              <p className="text-green-600 text-sm">Amount: ₹{Number(result.amount).toLocaleString('en-IN')}</p>
              <p className="text-green-600 text-sm">Remaining: ₹{Number(result.balanceAfter).toLocaleString('en-IN')}</p>
            </div>
          )}
